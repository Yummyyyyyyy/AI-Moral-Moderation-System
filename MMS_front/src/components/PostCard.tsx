...
function derive(item: FeedItem) {
  ...
  let typed: { type: HarmType; score: number; hint: string | null } | null = null;
  for (const l of item.labels) {
    if (l.stage === "binary") binary = { harmful: !!l.is_harmful, score: l.score };
    if (l.stage === "typed" && l.harm_type)
      typed = { type: l.harm_type, score: l.score, hint: l.strategy_hint };
  }
  return { binary, typed };
}
        ...
        {visibleReplies.length > 0 ? (
          <div className="thread">
            {visibleReplies.map((r) => (
              <ReplyRow key={r.id} reply={r} admin={admin} />
            ))}
          </div>
        ) : null}
